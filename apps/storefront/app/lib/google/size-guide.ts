...
export const transformGoogleSizeGuideData = (dataValues: string[][]) => {
    ...
    for (let ri = 0; ri < rest.length; ri++) {
        ...
        for (let hi = 0; hi < headersRest.length; hi++) {
            ...
            const notValid: boolean =
                APPARELS.includes(gender) && (hi <= 0 || hi > 7);
            if (notValid) {
                continue;
            }
...
export const normaliseGoogleSizeGuideData = (
    mapping: TableData,
    gender: string,
    categories: string[],
    unit: string
    ...
    const mappingGender: GenderAccessorData | undefined = Object.entries(
        mapping
    ).find(([key]) => {
    ...
    let mappingCategory: CategoryData | undefined =
        mappingGender[normalsizeCategory];
    if (!mappingCategory) {
    ...
    if (
        !data ||
        (data[ONE_SIZE] &&
            data[ONE_SIZE] instanceof String &&
            data[ONE_SIZE].toUpperCase() === 'TRUE')
    ) {
        return [];
    }
